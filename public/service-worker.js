...
self.addEventListener('push', function (e) {
    ...
    console.log("pusherMetadata", pusherMetadata)

    let db;
    const request = indexedDB.open("beams-".concat(pusherMetadata.instanceId));
    ...
    request.onsuccess = (event) => {
        ...
        // console.log("db", db)
        const request2 = db.transaction('beams').objectStore('beams').get(pusherMetadata.instanceId);
        ...
        request2.onsuccess = (event) => {
            ...
            setTimeout(() => {
                confirmReceive(request2.result.user_id, pusherMetadata.publishId)
            }, 2000)

            // console.log(`userid is ${request2.result.user_id}`);
...
self.addEventListener('notificationclick', function (e) {
    ...
    if (isPusherNotification) {
        ...
        // custom logic
        console.log("customerPayload click", pusher.customerPayload)
        console.log("pusherMetadata click", pusher.pusherMetadata)

        let db;
        const request = indexedDB.open("beams-".concat(pusher.pusherMetadata.instanceId));
        request.onerror = (event) => {
            console.error("Why didn't you allow my web app to use IndexedDB?!");
        };
        request.onsuccess = (event) => {
            db = event.target.result;
            console.log("db", db)
            const request2 = db.transaction('beams').objectStore('beams').get(pusher.pusherMetadata.instanceId);
            request2.onerror = (event) => {
                // Handle errors!
            };
            request2.onsuccess = (event) => {
                // Do something with the request.result!
                setTimeout(() => {
                    confirmOpen(request2.result.user_id, pusher.pusherMetadata.publishId)
...
PusherPushNotifications.onNotificationReceived = ({
    ...
    // console.log("local", window.localStorage.get("sessionID"))
...
const confirmReceive = async (user_id, publishId) => {
    console.log("confirming receive noti ")


    const endpoint = "https://api.freeanalytics.one";
    const headers = {
        "content-type": "application/json",
        // "Authorization": "<token>",
        credentials: "include",
    ...
    const graphqlQuery = {
        "operationName": "confirmNotiReceived",
        "query": `mutation confirmNotiReceived($userId: ID!, $publishId: ID!) {
            confirmNotiReceived(user_id: $userId, publishId: $publishId)
          }`,
        "variables": {
            "userId": user_id,
            "publishId": publishId,
    ...
    try {
        ...
        console.log("confirm receive status ", response)
...
const confirmOpen = async (user_id, publishId) => {
    console.log("confirming open noti ")
    ...
    const graphqlQuery = {
        "operationName": "confirmNotiOpened",
        "query": `mutation confirmNotiOpened($userId: ID!, $publishId: ID!) {
            confirmNotiOpened(user_id: $userId, publishId: $publishId)
          }`,
        "variables": {
            "userId": user_id,
            "publishId": publishId,
    ...
    try {
        const response = await fetch(endpoint, options);
        console.log("confirm open status", response)
